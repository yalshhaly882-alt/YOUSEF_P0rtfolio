...
(function(){
  ...
  btn.addEventListener('click', ()=>{
    ...
    localStorage.setItem('yn_theme', document.documentElement.classList.contains('light') ? 'light' : 'dark');
    updateIcon();
  });
})();
...
function ynApplyContent(data){
  ...
  if(data.artists) ynRenderArtists(data.artists);
}
...
function ynRenderSocialIcons(socials){
  ...
  if(socials.whatsapp){
    const digits = socials.whatsapp.replace(/[^0-9]/g, '');
    const wrap = document.getElementById('social-whatsapp-wrap');
    const link = document.getElementById('social-whatsapp');
    const numEl = document.getElementById('social-whatsapp-number');
    if(wrap && link && digits){
...
function ynRenderArtists(artists){
  const wrap = document.getElementById('artistsWrap');
  if(!wrap) return;
  if(!artists.length){
  ...
  artists.forEach(a=>{
    ...
    card.innerHTML = `
      ${a.photo ? `<img src="${a.photo}" alt="${ynEscapeHtml(a.name || '')}">` : ''}
      <div class="artist-name">${ynEscapeHtml(a.name || '')}</div>
    `;
...
(function(){
  // Used only to skip logging your own visits while testing edit mode —
  // real admin permission is checked separately via Firebase Auth below.
  ...
  if(typeof window.db !== 'undefined' && !isEditMode){
    ...
    if(/iPhone/i.test(ua)) os = 'iPhone';
      ...
      os = m && m[1] ? ('Android — ' + m[1].trim()) : 'Android';
    }
    ...
    msgForm.addEventListener('submit', (e)=>{
      ...
      window.db.ref('comments').push({ name, text, ts: Date.now() })
        .then(()=>{
          ...
          msgForm.reset();
          if(window.ynNotifyTelegram){
            window.ynNotifyTelegram('💬 تعليق جديد على موقعك من ' + name + ':\n' + text);
          }
        })
        .catch(()=>{
          statusEl.className = 'msg-status err';
...
// ---- Admin edit mode (only active for a real, signed-in Firebase Auth
// session — not just a URL parameter or a locally-stored flag) ----
(function(){
...
function startEditMode(){
  ...
  // Floating toolbar: bold / custom color / heading-style / spacing / save / logout
  const toolbar = document.createElement('div');
  toolbar.id = 'admin-toolbar';
  toolbar.innerHTML = `
    <button data-cmd="bold" title="عريض"><b>B</b></button>
    <input type="color" id="textColorPicker" class="custom-color" title="لوّن النص المحدد بأي لون" value="#7c6ff0">
    <button data-cmd="heading" title="حوّل لعنوان">H</button>
    <button data-cmd="space" title="زود مسافة هنا">↕️</button>
  ...
  function showToast(msg){
    toast.textContent = msg || ((typeof window.db !== 'undefined')
  ...
  // Real color picker — applies the chosen color to whatever text is
  // selected right now, any color (not just one fixed accent color).
  document.getElementById('textColorPicker').addEventListener('input', (e)=>{
    wrapSelection(null, e.target.value);
  });

  function wrapSelection(className, color){
  ...
  // Inserts a small vertical gap at the cursor position, so you can space
  // out text inside whatever block you're editing.
  ...
  document.getElementById('logoutBtn').addEventListener('click', ()=>{
    firebase.auth().signOut().then(()=>{ location.href = 'admin.html'; });
  });
