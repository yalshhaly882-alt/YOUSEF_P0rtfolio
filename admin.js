function show(id){
  ['notConfigured','loginScreen','dashboard'].forEach(x=>{
...
if(typeof window.db === 'undefined' || typeof firebase === 'undefined' || !firebase.auth){
  show('notConfigured');
} else {
  // Real login — a genuine Firebase Authentication account, not a password
  // stored in the database. Session persists automatically across visits.
  ...
  function doLogin(){
    ...
    firebase.auth().signInWithEmailAndPassword(email, pass)
      .catch(e=>{
        err.textContent = 'الإيميل أو الباسورد غلط، جرب تاني.';
      })
  ...
  document.getElementById('changePassBtn').addEventListener('click', ()=>{
    ...
    user.reauthenticateWithCredential(credential)
      ...
      .then(()=>{
        ...
        err.textContent = 'اتغيّر بنجاح على كل الأجهزة ✓';
...
document.getElementById('saveSocialsBtn').addEventListener('click', ()=>{
  const btn = document.getElementById('saveSocialsBtn');
  ...
  ynGetContent().then(data=>{
    ...
    msg.textContent = 'حصل خطأ، جرب تاني: ' + err.message;
  }).finally(()=>{
    btn.disabled = false;
...
function renderTrackList(tracks){
  ...
  tracks.forEach((t, idx)=>{
    ...
    `;
    const delBtn = div.querySelector('.mi-del');
    delBtn.addEventListener('click', ()=>{
      delBtn.disabled = true;
      ynGetContent().then(data=>{
        data.tracks = (data.tracks || []);
        data.tracks.splice(idx, 1);
        return ynSaveContent(data).then(()=> renderTrackList(data.tracks));
      }).catch(()=>{
        alert('حصل خطأ في الحذف، جرب تاني.');
...
document.getElementById('addTrackBtn').addEventListener('click', ()=>{
  ...
  if(!title || !url){
    alert('اكتب اسم الأغنية والرابط الأول.');
    return;
  }
  btn.disabled = true;
  const originalLabel = btn.textContent;
  ...
  ynGetContent().then(data=>{
    ...
    return ynSaveContent(data).then(()=>{
      renderTrackList(data.tracks);
      titleEl.value = '';
      urlEl.value = '';
    });
  }).catch(()=>{
    alert('حصل خطأ في الحفظ، جرب تاني.');
  }).finally(()=>{
    btn.disabled = false;
    btn.textContent = originalLabel;
  });
...
// ---- Favorite artists (name + photo, multiple) ----
...
function renderArtistList(artists){
  ...
  if(!artists.length){
    wrap.innerHTML = '<div class="msg-empty">لسه مفيش فنانين مضافين.</div>';
    return;
  }
  wrap.innerHTML = '';
  artists.forEach((a, idx)=>{
    ...
    div.innerHTML = `
      ...
      <span class="ai-name">${escapeHtml(a.name || 'بدون اسم')}</span>
      <button class="mi-del" data-idx="${idx}">🗑️ حذف</button>
    `;
    const delBtn = div.querySelector('.mi-del');
    delBtn.addEventListener('click', ()=>{
      ...
      ynGetContent().then(data=>{
        ...
        data.artists.splice(idx, 1);
        return ynSaveContent(data).then(()=> renderArtistList(data.artists));
      }).catch(()=>{
        alert('حصل خطأ في الحذف، جرب تاني.');
        delBtn.disabled = false;
      });
...
document.getElementById('addArtistBtn').addEventListener('click', ()=>{
  ...
  const photoEl = document.getElementById('artistPhotoInput');
  const btn = document.getElementById('addArtistBtn');
  const name = nameEl.value.trim();
  if(!name){
    alert('اكتب اسم الفنان الأول.');
    return;
  }
  ...
  function save(photoData){
    ...
    ynGetContent().then(data=>{
      data.artists = data.artists || [];
      data.artists.push({ name, photo: photoData || '' });
      return ynSaveContent(data).then(()=>{
        renderArtistList(data.artists);
        nameEl.value = '';
        photoEl.value = '';
  ...
  if(visitsSummary && visitsList){
    db.ref('visits').on('value', snap=>{
      ...
      items.slice(0, 50).forEach(v=>{
        ...
        row.innerHTML = `
          <div class="mi-top">
            <span class="mi-name">${v.device === 'mobile' ? '📱' : '💻'} ${escapeHtml(v.os || '')} · ${escapeHtml(v.browser || '')}</span>
